...
class DBUtils:
    ...
    def _get_prediction_table_columns(self, conn: Optional[Connection] = None) -> Set[str]:
        """
        获取 t_prediction_parameters 的字段集合（带缓存，10分钟过期）
        """
        now = time.time()
        if self._pred_table_cols_cache and (now - self._pred_table_cols_cache_ts) < self._pred_table_cols_cache_ttl:
            return self._pred_table_cols_cache
        ...
        try:
            ...
            self._pred_table_cols_cache = cols
            self._pred_table_cols_cache_ts = now
            return cols
        except Exception as e:
            logger.warning(f"读取 {self.TABLE_PRED} 字段失败：{str(e)}")
            return set()
        finally:
            if close_after:
                try:
    ...
    def get_latest_feature_cache_by_group(self, workface_id: int, spatiotemporal_group: str,
        ...
        目的：避免预测阶段历史/趋势/时间增强特征全补0，导致线上分布与训练不一致。
        """
        try:
    ...
    def get_recent_targets_by_group(
        ...
        try:
            ...
            with self.engine.connect() as conn:
                try:
                    rows = conn.execute(sql_join, {"wid": wid, "grp": grp, "dt": dt}).fetchall()
                except Exception:
                    rows = []

                if (not rows) and sql_fallback is not None:
                    try:
                        rows = conn.execute(sql_fallback, {"wid": wid, "grp": grp, "dt": dt}).fetchall()
                    except Exception:
                        rows = []
    ...
    def fetch_recent_training_window_with_features(
        ...
        try:
            ...
            # -------------------------
            # 3) 列集合（关键：彻底剔除 gas_emission_q）
            # -------------------------
            # feature_cache 默认增强列：不包含 gas_emission_q_* 任何字段
            f_cols_default = [
                "coord_hash",
            ...
            # pred 核心列：目标只保留 S、q（不含 gas_emission_q）
            p_cols_default = [
                "id",
                "working_face",
                "workface_id",
                ...
                "distance_to_face",
                "drilling_depth",
                "regional_measure_strength",
                "x_coord",
                "y_coord",
                "z_coord",
                "fault_influence_strength",
                "face_advance_distance",
                "distance_from_entrance",
                # 目标列（训练y用）
                "drilling_cuttings_s",
                "gas_emission_velocity_q",
            ]

            # 如果上层传了 needed_cols：按 needed_cols 裁剪，但强制删除 gas_emission_q
            if needed_cols:
                try:
                    need = [str(c).strip() for c in needed_cols if isinstance(c, str) and str(c).strip() != ""]
                except Exception:
                    need = []

                # ---- 关键：无条件丢弃 gas_emission_q 及其派生列（100% 不引用）----
                banned_prefix = ("gas_emission_q", "gas_emission_q_")
                need = [c for c in need if not (c == "gas_emission_q" or c.startswith("gas_emission_q_"))]

                # 目标列强制保留（同样不包含 gas_emission_q）
                for t in ["drilling_cuttings_s", "gas_emission_velocity_q"]:
                    if t not in need:
                        need.append(t)

                # 拆分 pred/feature
                need_set = set(need)
                f_cols = [c for c in f_cols_default if c in need_set]
                p_cols = [c for c in p_cols_default if c in need_set] or p_cols_default
            else:
                f_cols = f_cols_default
                p_cols = p_cols_default
